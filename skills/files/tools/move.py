"""Move or rename. Moving onto an existing path is an overwrite (YELLOW)."""
...
def precheck(p: P, env: ToolEnv) -> None:
    ...
    env.paths.check_delete(src)            # the source disappears from its place
...
def action(p: P, env: ToolEnv) -> str:
    return "file.overwrite" if env.paths.resolve(p.destination).exists() else "file.move"
...
async def run(p: P, env: ToolEnv) -> ToolResult:
    ...
    return ToolResult(True, f"moved {src} → {dst}", {"destination": str(dst)},
                      {"source_exists": src.exists(), "destination_exists": dst.exists()})


TOOL = Tool(name="move", params=P, run=run, action=action,
