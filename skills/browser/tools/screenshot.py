...
async def run(p: P, env: ToolEnv) -> ToolResult:
    r = await browser_call(env, "POST", f"/v1/sessions/{p.session_id}/screenshot")
    if isinstance(r, ToolResult):
        return r
    return ToolResult(True, f"page screenshot of {r.get('title')!r}", r,
                      page_evidence(r), untrusted=True)


TOOL = Tool(name="screenshot", params=P, run=run, action="browser.read",
